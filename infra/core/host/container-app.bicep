...
resource app 'Microsoft.App/containerApps@2024-03-01' = {
  ...
  // https://github.com/Azure/azure-dev/blob/2374456c44ab2d20605c4d31c25d1ffe23fec343/templates/common/infra/bicep/core/host/container-app.bicep#L100-L103
  // https://github.com/microsoft/azure-container-apps/issues/836
  // User Managed Identifyを使う場合、ACRPullを与えられるので、わざわざダミーのpublicイメージをデプロイする必要はないはず。
  ...
  properties: {
    ...
    template: {
      ...
      containers: [
        {
          // TODO: コンテナを空でデプロイするとなにか問題があるのかを確認。問題なければ、empty(imageName) の場合は空配列にする。
